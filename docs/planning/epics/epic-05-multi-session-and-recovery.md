...
- [x] **Story 5.4 — Control plane reconcile + UI status** (~2h) — *backend-only; React UI + Resume endpoint deferred*
  ...
  - *Files:* `apps/mars-control/backend/src/mars_control/sessions/__init__.py`, `apps/mars-control/backend/src/mars_control/sessions/reconcile.py`, `tests/control/test_reconcile.py`
  - *Done when:* clicking Resume on a `needs_restart` session spawns a new subprocess and the card flips to `running`
  - *Outcome:* Shipped the **pure reconcile half**. `reconcile_machine_state(known, machine)` is a pure function that diffs two iterables of typed session states and returns a structured `ReconcileReport` with four buckets: `in_sync` (matching session_id + status), `status_changes` (same id, different status — machine wins as authoritative), `disappeared` (known to control plane but not reported by machine — mark needs_restart), `appeared` (machine reports but control plane missed — adopt). `KnownSessionState` / `MachineSessionState` / `ReconcileStatusChange` are all frozen dataclasses so the report can be passed around without accidental mutation. `has_divergence` / `divergence_count` properties. Zero I/O, zero state — the caller decides how to act on the report. The epic's "NEVER auto-resume" rule is preserved: this module only *describes* divergence, it does not call any endpoint. 14 unit tests covering empty inputs, in-sync happy path, each divergence bucket individually, mixed 4-status reconcile, mutation safety, generator consumption, property edge cases. **Deferred**: (1) the React `SessionCard.tsx` UI — blocks on Epic 4 frontend scaffold; (2) the `/agents/{name}/resume` control-plane endpoint that calls the supervisor's `POST /sessions` — the supervisor already accepts that POST via Story 1.5 so the endpoint is a thin proxy that lands with Epic 4's UI work; (3) the mars-control SQLite sessions table — Epic 4 dashboard story introduces it. Full suite: 466 passed, 1 skipped. **29/47 stories done. Epic 5 complete (4/4 backend).**
