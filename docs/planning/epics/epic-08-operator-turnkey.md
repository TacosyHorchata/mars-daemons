...
- [x] **Story 8.2 — Templates tab + discovery API** (~1h)
  - *Goal:* Dashboard Templates tab with card grid rendering templates from `GET /templates`.
  - *Files:* `apps/mars-control/backend/src/mars_control/templates/__init__.py`, `apps/mars-control/backend/src/mars_control/api/routes.py`, `apps/mars-control/frontend/src/components/templates/TemplateList.tsx`, `apps/mars-control/frontend/src/components/templates/TemplateCard.tsx`, `apps/mars-control/frontend/src/app/dashboard/page.tsx`, `apps/mars-control/frontend/src/lib/api.ts`, `tests/control/test_templates_discovery.py`
  - *Done when:* Templates tab shows the Tracker Ops Assistant card with a working Start button
  - *Outcome:* **Backend** — new `mars_control.templates` package with `TemplateSummary` dataclass + `discover_templates(dir)` scanner that glob-sorts `*.yaml`, loads each via `AgentConfig.from_yaml_file`, and silently skips broken entries (tests assert that two garbage files next to a valid one still return the valid one — one broken template cannot take down the dashboard). Prompt-preview is the first non-blank non-heading line of the `.prompt.md` sibling, capped at 200 chars. `GET /templates` wired under `_require_current_user` (behind auth like the session proxy), `template_dir` kwarg on `create_control_app` with `MARS_TEMPLATE_DIR` env fallback → repo-relative `templates/` default. **Frontend** — `listTemplates()` + `TemplateSummary` type in `lib/api.ts`; `TemplateCard` renders name + description + italic prompt preview + MCP chip row + Spanish "Empezar" button; `TemplateList` handles loading/empty/error states with Spanish copy; dashboard `page.tsx` adds a two-tab switcher (`Daemons` | `Plantillas`) between the header and the grid. For v1 the Empezar button fires an alert + console.info; Story 8.3 replaces that with the OnboardingWizard modal. 10 new backend unit tests (discover happy/empty/broken-yaml/sorted/single-file/to_dict shape + endpoint 200/401/empty/real-template). Frontend builds clean with all 7 routes.

- [x] **Story 8.3 — OnboardingWizard steps 1–3** (~2h)
  ...
  - *Files:* `apps/mars-control/frontend/src/components/templates/OnboardingWizard.tsx`, `apps/mars-control/frontend/src/app/dashboard/page.tsx`
  - *Done when:* steps 1–3 navigate forward/back cleanly on mobile viewport
  - *Outcome:* **`OnboardingWizard.tsx`** is a single self-contained client component (~350 lines) with a six-step progress bar, per-step copy in Spanish-mexicano, Escape-to-close, click-outside-to-close, and auto-reset-to-step-1 when a new template opens. **Step 1 (welcome)** frames the agent's value in operator language — "un agente que lee WhatsApp, consulta tu CRM y te ayuda a cerrar pendientes" — plus the time estimate (5-10 min) and honest Claude Max $200/mo disclosure up front. **Step 2 (Claude account)** opens `claude.ai/signup` in a new tab behind a prominent secondary link and presents "Ya tengo cuenta" as the advance button. **Step 3 (Claude Max)** opens `claude.ai/upgrade` the same way, explicitly clarifies that Mars never touches the card ("el cobro lo hace Anthropic directamente"). **Step 4 (placeholder)** is a deliberate "Próximamente" card so the wizard doesn't dead-end in v1 — Story 8.4 replaces it with the OAuth/secrets/deploy flow. Reusable `PrimaryButton`/`SecondaryButton`/`SecondaryLinkButton`/`WizardNav` primitives live in the same file for now (no shared design system yet, Malix adoption is v1.1). Dashboard page wires the wizard via a `wizardTemplate` useState — `onStart` sets the template, `onClose` clears it, the modal is conditionally rendered at the tree root so it overlays both tabs. Next.js build green with 7 routes.
